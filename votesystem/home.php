...
<div class="wrapper">
                    ...
                    <div class="col-sm-10 col-sm-offset-1">
                        ...
                        <?php
                            ...
                            if(pg_num_rows($vquery) > 0){
                                ...
                                <form method="POST" id="ballotForm" action="submit_ballot.php">
                                    <?php
                                        ...
                                        $query = $conn->pg_query($conn, $sql);
                                        while($row = pg_fetch_assoc($query)){
                                            $sql = "SELECT * FROM candidates WHERE position_id='".$row['id']."'";
                                            $cquery = $conn->pg_query($conn, $sql);
                                            while($crow = pg_fetch_assoc($cquery)){
                         ...
                         <h3 class="box-title"><b>'.$row['description'].'</b></h3>
                                                            ...
                                                            <div class="box-body">
                                                                <p>'.$instruct.'</p>
                                                                <ul id="sortable'.$row['id'].'" class="list-group checked-list-box">
                                                                    '.$candidate.'
                                                                </ul>
                                                            </div>
                                                        </div>
                                        ...
                                        if(!isset($_SESSION['post'])){
                                            echo '
                                                <div class="text-center">
                                                    <button type="button" class="btn btn-success btn-curve btn-flat" style="background-color: #4682B4 ;color:black ; font-size: 15px; font-family:Times" id="preview"><i class="fa fa-file-text"></i> Preview</button>
                                                    <button type="submit" class="btn btn-curve btn-flat" style="background-color: #4682B4 ;color:black ; font-size: 15px; font-family:Times" name="vote"><i class="fa fa-check-square-o"></i> Submit</button>
                                                </div>
                                            ';
                                        }
    ...
    <?php include 'includes/vote_modal.php'; ?>
</div>
